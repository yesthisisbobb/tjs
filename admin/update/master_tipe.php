...
<div class="row">
    <div class="col-sm-3">Product Group
      <select name="kodegrup" id="kodegrup" class="form-control">
        <?php
          $sql1pg = "SELECT * FROM detail_sub_grup where kode='$namag'";
               $querypg = mysqli_query($conn, $sql1pg);
               while($amkupg = mysqli_fetch_array($querypg)){
                 echo '<option>'.$amkupg['nama'].'</option>';
            }
          ?>
      <?php
        $sqlpgg = "SELECT * FROM detail_sub_grup";
             ...
             while($amkupgg = mysqli_fetch_array($querypgg)){
               echo '<option value='.$amkupgg['nama'].'>'.$amkupgg['nama'].'</option>';
          }
        ?>
      </select>
  </div>
    ...
    <?php
      $sqlpbc = "SELECT * FROM master_divisi";
           ...
           while($amkupbc = mysqli_fetch_array($querypbc)){
             echo '<option value='.$amkupbc['kode'].'>'.$amkupbc['nama'].'</option>';
        }
      ?>
    </select>
...
<div class="col-sm-3"> Product Business Category
  <select name="bc1" id="bc1" class="form-control">
    <?php
      $sqlb = "SELECT * FROM master_perusahaan where kode='$bc1'";
           $queryb = mysqli_query($conn, $sqlb);
           while($amkub = mysqli_fetch_array($queryb)){
             echo '<option>'.$amkub['nm_perusahaan'].'</option>';
        }
      ?>
  ...
  <?php
    $sqlbb = "SELECT * FROM master_perusahaan";
         ...
         while($amkubb = mysqli_fetch_array($querybb)){
           echo '<option value='.$amkubb['kode'].'>'.$amkubb['nm_perusahaan'].'</option>';
      }
    ?>
  </select>
</div>
  ...
  <div class="row">
    ...
    <div class="row">
        ...
        <input type="text" class="bundar form-control" id="nama" name="nama" placeholder="Type Name" value=<?php echo $namap; ?> required>
          ...
          <div class="row">
          <div class="col-sm-2">Glossiness Level
            <div class="form-group">
              <select name="glosy" id="glosy" class="form-control">
                <?php
                  $sqlg = "SELECT * FROM master_glosy where id='$glosy'";
                       $queryg = mysqli_query($conn, $sqlg);
                       while($amkug = mysqli_fetch_array($queryg)){
                         echo '<option>'.$amkug['nama'].'</option>';
                    }
                  ?>

              <?php
                $sqlgg = "SELECT * FROM master_glosy";
                     $querygg = mysqli_query($conn, $sqlgg);
                     while($amkugg = mysqli_fetch_array($querygg)){
                       echo '<option value='.$amkugg['nama'].'>'.$amkugg['nama'].'</option>';
                  }
                ?>
              </select>
        </div>
          ...
          <div class="col-sm-2"> Surfaces
            <div class="form-group">
              <select name="surface" id="surface" class="form-control">
                <?php
                  $sqls = "SELECT * FROM master_surface where id='$surface'";
          ...
          <div class="col-sm-2">Color
            ...
            <select name="color" id="color" class="form-control">
              <?php
                $sqlc = "SELECT * FROM master_color where id='$color'";
                     $queryc = mysqli_query($conn, $sqlc);
                     while($amkuc = mysqli_fetch_array($queryc)){
                       echo '<option>'.$amkuc['nama'].'</option>';
                  }
                ?>
            <?php
              $sqlcc = "SELECT * FROM master_color";
                   ...
                   while($amkucc = mysqli_fetch_array($querycc)){
                     echo '<option value='.$amkucc['id'].'>'.$amkucc['nama'].'</option>';
                }
              ?>
            </select>
        </div>
          ...
          <div class="row">
              ...
              <div class="col-sm-2">Pattern
                ...
                <select name="pattern" id="pattern" class="form-control">
                  <?php
                    $sqlp = "SELECT * FROM master_pattern where id='$pattern'";
                         $queryp = mysqli_query($conn, $sqlp);
                         while($amkup = mysqli_fetch_array($queryp)){
                           echo '<option>'.$amkup['nama'].'</option>';
                      }
                    ?>
                ...
                <?php
                  $sqlpt = "SELECT * FROM master_pattern";
                       ...
                       while($amkupt = mysqli_fetch_array($querypt)){
                         echo '<option value='.$amkupt['nama'].'>'.$amkupt['nama'].'</option>';
                    }
                  ?>
                </select>
            </div>
          ...
          <div class="col-sm-2">Buying Unit
              ...
              <?php
                $sqlbuyunit = "SELECT * FROM master_unit";
                     ...
                     while($amkubuyunit = mysqli_fetch_array($querybuyunit)){
                       echo '<option value='.$amkubuyunit['nama'].'>'.$amkubuyunit['nama'].'</option>';
                  }
                ?>
              </select>
            ...
            <div class="col-sm-2">Loading Limit
              <select name="ll" id="ll" class="form-control">
                <?php
                  $sql1 = "SELECT * FROM master_ll where id='$ll'";
                       $query1 = mysqli_query($conn, $sql1);
                       while($amku1 = mysqli_fetch_array($query1)){
                         echo '<option>'.$amku1['nama'].'</option>';
                    }
                                        ...
                                        <div class="row">
                                            ...
                                            <input type="text" class="bundar form-control" id="nama" name="nama" placeholder="Type Name" required>
                                              ...
                                              <div class="col-sm-2">
                                                <div class="form-group">
                                                  <select name="glosy" id="glosy" class="form-control">
                                                    <option>Glossiness Level</option>
                                                  <?php
                                                    $sql = "SELECT * FROM master_glosy";
                                                         $query = mysqli_query($conn, $sql);
                                                         while($amku = mysqli_fetch_array($query)){
                                                           echo '<option value='.$amku['id'].'>'.$amku['nama'].'</option>';
                                                      }
