       ...
       <table id="example" class="table table-striped table-bordered">
         <thead>
           <tr>
             ...
             <th class="text-nowrap">Invoice No.</th>
               ...
               <th class="text-nowrap">View Invoice</th>
         ...
         <tbody>
           ...
           $un1=$_SESSION["username"];
             $sql = "SELECT * FROM jual where user_id='$un1'";
                  $query = mysqli_query($conn, $sql);
                  while($amku = mysqli_fetch_array($query)){
                    $cartidku=$amku['cart_id'];
                    $noku=$amku['no'];
                    $gt=$gt+$amku['grand_total'];
                    $h=0;
                    $statuspayment=$amku['status_payment'];
           ...
           <tr class="odd gradeX">
             ...
             <td class="with-btn" nowrap="">
               <a href="invlistdtl.php?no=<?php echo $cartidku; ?>" target="_blank" class='btn btn-primary btn-small'>Invoice</a>
             </td><td> <?php if ($statuspayment=="Paid") {?>

                 <?php
                     $sql11 = "SELECT DISTINCT nodo FROM dodtl where no_cart='$cartidku'";
                        $query11 = mysqli_query($conn, $sql11);
                        while($amku11 = mysqli_fetch_array($query11)){
                           $nodo=$amku11['nodo'];
                           // echo $nodo;
                              $sql111 = "SELECT * from do where nodo='$nodo'";
                              $query111 = mysqli_query($conn, $sql111);
                              while($amku111 = mysqli_fetch_array($query111)){
                                $tglkirim1=$amku111['tglkirim'];
                              }

                   ?>
                   <a href="dolistdtl.php?so=<?php echo $cartidku; ?>&no=<?php echo $nodo; ?>" target="_blank"><?php echo "| ". $tglkirim1; ?></a>
               <?php }?>
