...
if ($aksi=="delete")
{
	$sqlbalik= "SELECT * from cartdtl where no='$no'";
	$querybalik = mysqli_query($conn, $sqlbalik);
	 while($amkubalik = mysqli_fetch_array($querybalik)){
		 $kode=$amkubalik['kode_stok'];
		 $shading=$amkubalik['shading'];
		 $jumlah=$amkubalik['jumlah'];
	 }
	 $sqlbalik1= "SELECT * from master_shading where kode_stok='$kode' and kode_shading='$shading'";
 	 $querybalik1 = mysqli_query($conn, $sqlbalik1);
 	 while($amkubalik1 = mysqli_fetch_array($querybalik1)){
 		 $jumlah1=$amkubalik1['awal'];
 	 }
	 $kembali=$jumlah+$jumlah1;

	 $sql41= "UPDATE master_shading SET awal='$kembali' where kode_stok='$kode' and kode_shading='$shading'";
...
else if ($aksi=="delete1")
{
	$sql41= "DELETE from icartdtl where no='$no'";
	$query41 = mysqli_query($conn, $sql41);
	if( $query41 )
{
			header('Location: index.php?status=sukses');
