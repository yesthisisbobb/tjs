...
if(isset($_POST["login"])){
    ...
    //maka jalankan script ini.
     $name =$_POST['username']; //$name yg di isi dng value username dari form login
     $password = $_POST['password']; //$pass yg di isi dng value password dari form login lalu hash dengan md5
     ...
     if(mysqli_num_rows($login) ===1){
      ...
      $_SESSION["id"] = $data["id"]; //buat session level
     		header('Location: '.$base_url); //redirect kehalaman admin.
    ...
    else if($password == $data["password"] && $data["level"] == "user" && $data["role"] == "sales"){
     		$_SESSION["username"] = $data["email"]; //buat session username
     		$_SESSION["level"] = $data["level"]; //buat session level
     		header('Location: '.$base_url.'/sales/'); //redirect kehalaman user.
...
///////////////////////////////////////
  if(isset($_POST["tambah_pengguna"])){
   if($_POST["email"] =="" | $_POST["username"] == "" | $_POST["password"] == "" | $_POST["level"] == ""){
      echo'<script>
             alert("Jangan ada yang kosong !");
             windows.location="'.$base_url.'/admin/tambah-pengguna.php";
            </script>';
      return false;
   }
   $email = $_POST["email"];
...
//////////////////////////////////////
  if(isset($_SESSION["username"])){
	  ...
	  $data = mysqli_query($conn,"SELECT * FROM login WHERE email='$username'");
      ...
      if($_SESSION["level"] == "admin"){
        ...
        $count = mysqli_query($conn,"SELECT * FROM master_retail ORDER BY id_toko DESC");
        $totalPenggunaretail = mysqli_num_rows($count); //total pengguna

        $count = mysqli_query($conn,"SELECT * FROM detail_toko ORDER BY id_toko DESC");
        $totalPenggunatoko = mysqli_num_rows($count); //total pengguna

        $count = mysqli_query($conn,"SELECT * FROM detail_project ORDER BY id_toko DESC");
        $totalPenggunaproject = mysqli_num_rows($count); //total pengguna
