...
function smtp_mail($to, $subject, $message, $from_name, $from, $cc, $bcc, $debug=false) {
  ...
  $mail->SMTPAuth 	= true;                        				
  $mail->Host 		  = 'mail.alazharyis.sch.id';  // Masukkan Server SMTP
  $mail->Port 		  = 465;                                      // Masukkan Port SMTP
  $mail->SMTPSecure = 'ssl';                                    // Masukkan Pilihan Enkripsi ( `tls` atau `ssl` )
  $mail->Username 	= 'admin@alazharyis.sch.id';                // Masukkan Email yang digunakan selama proses pengiriman email via SMTP
  $mail->Password 	= 'Plafonrusak123';        						          // Masukkan Password dari Email tsb
  $default_email_from       = 'admin@alazharyis.sch.id';        // Masukkan default from pada email
  $default_email_from_name  = 'Karya Modern';           // Masukkan default nama dari from pada email
